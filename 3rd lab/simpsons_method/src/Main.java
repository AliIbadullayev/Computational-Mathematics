...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        simpsonMethod(a,b,epsilon,n,iterations,h,afterComma,result,resultPrev,resultCurr);
    }
    
    private static void simpsonMethod(double a, double b, double epsilon, int n, int iterations, double h, int afterComma, double result, double resultPrev, double resultCurr){
        while (Math.abs(resultCurr - resultPrev)/(Math.pow(2,K) - 1) > epsilon){
            result = h/3*(func.apply(a)+func.apply(b));

            for(int i = 1; i<n; i++){
                if (i%2==0) {
                    result = result + h/3 * 2* func.apply(a+(i)*h);
                }
                else {
                    result = result + h/3 * 4 * func.apply(a+i*h);
                }
            }
